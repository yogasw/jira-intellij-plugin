...
public class FieldEditorFactory {
    ...
    public static FieldEditor create(JiraIssueFieldProperties properties, @Nullable JiraIssue issue) {
        ...
        if (TEXT_FIELDS.contains(fieldName)) {
            return new TextFieldEditor(properties.getName(), isNull(issue) ? null : issue.getAsString(fieldName), properties.isRequired());
        } else if (TEXT_AREA_FIELDS.contains(fieldName)) {
            return new TextAreaFieldEditor(properties.getName(), isNull(issue) ? null : issue.getAsString(fieldName), properties.isRequired());
        } else if (DATE_FIELDS.contains(fieldName)) {
            return new DateFieldEditor(properties.getName(), isNull(issue) ? null : issue.getAsDate(fieldName), properties.isRequired());
        } else if (USER_PICKER_FIELDS.contains(fieldName)) {
            return new UserSelectFieldEditor(properties.getName(), isNull(issue) ? null : issue.getAsJiraIssueUser(fieldName), properties.isRequired());
    ...
    private static FieldEditor createCustomFieldEditor(JiraIssueFieldProperties properties, @Nullable JiraIssue issue) {
        ...
        if (PROJECT.equals(type)) {
            ...
            return new ProjectSelectFieldEditor(properties.getName(), isNull(issue) ? null : issue.getCustomfieldValue(properties.getSchema().getCustomId()), properties.isRequired(), isArray, projects);
        } else if (VERSION.equals(type)) {
            List<JiraProjectVersion> versions = JiraGsonUtil.getAsList(values, JiraProjectVersion[].class);
            return new VersionSelectFieldEditor(properties.getName(), isNull(issue) ? null : issue.getCustomfieldValue(properties.getSchema().getCustomId()), properties.isRequired(), isArray, versions);
        }
